...
class emuForm extends FormApplication {
    ...
    return mergeObject(
      {
        themePresetList: {
          ...
          'gates-season-2': game.i18n.localize('emu.theme-preset-gates-season-2'),
        },
  ...
  getThemePreset(formData) {
    ...
    if ($('select[name="themePreset"]').val() === 'gates-season-2') {
      for (const [key, value] of Object.entries(THEME.GATE_SEASON_2)) { $(`input[name="${key}"]`).prop('value', value); }
    }
  }
